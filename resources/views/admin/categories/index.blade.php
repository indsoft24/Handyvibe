        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div class="flex items-center space-x-3">
                ...
                <div>
                    ...
                    <p class="text-sm text-gray-600 dark:text-gray-400">Manage your product categories and organize your inventory</p>
                </div>
            </div>
    ...
    <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden">
        <div class="border-b border-gray-200 dark:border-gray-700 px-6 py-4 bg-gray-50 dark:bg-gray-700">
            <div class="flex items-center justify-between">
                <h3 class="font-semibold text-lg text-gray-900 dark:text-white">All Categories</h3>
        ...
        <div class="p-6">
                        ...
                        <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                            @foreach ($categories as $category)
                                <tr class="hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors duration-150">
                                    <td class="py-4 px-4 font-medium text-gray-900 dark:text-white">{{ $category->id }}</td>
                                    <td class="py-4 px-4">
                                        @if ($category->image)
                                    ...
                                    <td class="py-4 px-4">
                                        <div class="flex flex-col">
                                            ...
                                            @if ($category->description)
                                                <span class="text-xs text-gray-500 dark:text-gray-400 mt-1">{{ Str::limit($category->description, 40) }}</span>
                                            @endif
                                        </div>
                                    </td>
                                    <td class="py-4 px-4">
                                        <code class="text-xs text-gray-600 dark:text-gray-300 bg-gray-100 dark:bg-gray-700 px-2 py-1 rounded">{{ $category->slug }}</code>
                                    </td>
                                    <td class="py-4 px-4">
                                    ...
                                    <td class="py-4 px-4">
                                        <form method="POST" action="{{ route('admin.categories.toggle-status', $category) }}" class="inline">
                                            ...
                                            <button type="submit" class="inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium transition-colors
                                                ...
                                                <span class="w-1.5 h-1.5 rounded-full mr-1.5 {{ $category->status ? 'bg-green-500' : 'bg-red-500' }}"></span>
                                                {{ $category->status ? 'Active' : 'Inactive' }}
                                            </button>
                                        </form>
                                    </td>
                                    <td class="py-4 px-4 text-sm text-gray-900 dark:text-white">{{ $category->sort_order }}</td>
            ...
            @else
                <div class="flex flex-col items-center justify-center text-center py-16">
                    ...
                    <p class="text-sm text-gray-500 dark:text-gray-400 mb-6">Get started by creating your first category to organize your products.</p>
                    <a href="{{ route('admin.categories.create') }}"
